...
evaluate every candidate independently, on its own merits, against these five checks:
...
5. reject if it packs more than two distinct technical claims into one post. most opinion candidates default to three dense clauses because there's room in 240 characters — that's compression, not concision. two claims with room to breathe beats three that don't fit comfortably. a short post that's well under the limit and says one clear thing is not a failure to fix; the character limit is a ceiling, not a target to fill.

give every candidate its own verdict and its own one-sentence reason, even the ones that pass. then, among the candidates you accepted (if any), pick the single strongest as the winner. if you accepted none, winner_index is null. do not pick a winner out of obligation — only from candidates that actually cleared all five checks.`;
